...
DAT_JSON_PATH = "/var/www/gaia_stats/dat.json"
...
def update_all():
    ...
    # "98.0.4758.102"
    driver = webdriver.Chrome(ChromeDriverManager("100.0.4896.60", chrome_type=ChromeType.CHROMIUM).install(),
                              options=op)
    ...
    with open("/home/auto_update/game_names.txt", "r") as names_file:
    ...
    with open("/home/auto_update/game_names.txt", "a") as names_file:
    ...
    errors_set = dict()
    two_players = pd.read_csv("/home/auto_update/two_players_data.csv")
    three_players = pd.read_csv("/home/auto_update/three_players_data.csv")
    four_players = pd.read_csv("/home/auto_update/four_players_data.csv")
    ...
    with open('/var/www/gaia_stats/errors/index.html', 'w') as file:
        html = "<html><head></head><body><p>" + error_string + "</p></body></html>"
        file.write(html)

    with open('/var/www/gaia_stats/js/vars.js', 'w') as js_file:
        js = 'const parsed = ' + str(len(two_players) + len(three_players) + len(four_players)) + ';\n' + 'const ' \
                                                                                                          'millis = ' \
             + str(round(time.time() * 1000))
        js_file.write(js)

    two_players.to_csv("/home/auto_update/two_players_data.csv", index=False)
    three_players.to_csv("/home/auto_update/three_players_data.csv", index=False)
    four_players.to_csv("/home/auto_update/four_players_data.csv", index=False)
...
def plot_adv_freq_remote(num_players, dat_x):
    ...
    for faction in factions:
        ...
        fig.write_html(
            "/var/www/gaia_stats/" + str(
                num_players) + "p/adv_techs_analysis/taken_freq/" + faction + ".html")


def plot_adv_track_remote(num_players, dat_x):
    # load json with overall results and make keys if empty
    dat_json = dict()
    if os.path.exists(DAT_JSON_PATH):
        with open(DAT_JSON_PATH, 'r') as j_dat:
            dat_json = json.load(j_dat)

    if not str(num_players) in dat_json.keys():
        dat_json[str(num_players)] = dict()

    for f in factions:
        if f not in dat_json[str(num_players)].keys():
            dat_json[str(num_players)][f] = dict()

        if not 'advanced_tech' in dat_json[str(num_players)][f].keys():
            dat_json[str(num_players)][f]['advanced_tech'] = dict()

        for t in list(adv_techs.values()):
            if t not in dat_json[str(num_players)][f]['advanced_tech'].keys():
                dat_json[str(num_players)][f]['advanced_tech'][t] = dict()
                tech_slots = [
                    "tech_adv-terra",
                    "tech_adv-nav",
                    "tech_adv-int",
                ...
                for s in tech_slots:
                    dat_json[str(num_players)][f]['advanced_tech'][t][s] = dict()
                    dat_json[str(num_players)][f]['advanced_tech'][t][s]['score'] = 0
                    dat_json[str(num_players)][f]['advanced_tech'][t][s]['count'] = 0
        ...
        for tech in adv_techs.values():
            ...
            for i, slot in enumerate(tech_slots):
                ...
                if pos_taken > 0:
                    ...
                    dat_json[str(num_players)][faction]['advanced_tech'][tech][slot]['score'] = taken_score / pos_taken
                ...
                dat_json[str(num_players)][faction]['advanced_tech'][tech][slot]['count'] = taken_amount
                dat_json[str(num_players)][faction]['advanced_tech'][tech][slot]['missed'] = not_taken_amount
            slot_scores[len(tech_slots)].append(tot_not_taken_score / not_taken_slot_count)
            dat_json[str(num_players)][faction]['advanced_tech'][tech]['tech_adv-not_taken'][
                'score'] = tot_not_taken_score / not_taken_slot_count
            game_nums[len(tech_slots)].append(tot_not_taken_amount)
            dat_json[str(num_players)][faction]['advanced_tech'][tech]['tech_adv-not_taken'][
                'count'] = tot_not_taken_amount
            slot_scores[len(tech_slots) + 1].append(tot_taken_score / tot_taken_slot_count)
            dat_json[str(num_players)][faction]['advanced_tech'][tech]['tech_adv-taken'][
                'score'] = tot_taken_score / tot_taken_slot_count
            game_nums[len(tech_slots) + 1].append(tot_taken_amount)
            dat_json[str(num_players)][faction]['advanced_tech'][tech]['tech_adv-taken']['count'] = tot_taken_amount
    ...
    with open(DAT_JSON_PATH, 'w') as j_file:
        json.dump(dat_json, j_file)
    ...
    for faction in factions:
        ...
        for tech in techs.values():
            ...
            for i, slot in enumerate(tech_slots):
                ...
                game_nums[i].append(taken_amount)
            if not_taken_slot_count > 0:
                slot_scores[len(tech_slots)].append(tot_not_taken_score / not_taken_slot_count)
            else:
                slot_scores[len(tech_slots)].append(None)
            game_nums[len(tech_slots)].append(tot_not_taken_amount)
            if tot_taken_slot_count > 0:
                slot_scores[len(tech_slots) + 1].append(tot_taken_score / tot_taken_slot_count)
            else:
                slot_scores[len(tech_slots) + 1].append(None)
            game_nums[len(tech_slots) + 1].append(tot_taken_amount)

        tech_slots.append("tech_not_taken")
        tech_slots.append("tech_taken")
        ...
        for i, slot in enumerate(tech_slots):
            fig.add_trace(go.Scatter(
                ...
                name=slot.split('_')[1],
                hovertemplate=
                '<br><b>Position</b>: ' + slot.split('_')[1] +
                '<br><b>Tech</b>: %{y}' +
                '<br><b>Ave Score</b>: %{x}' +
                '<br><b>Num Found</b>: %{text}' +
                '<extra></extra>',
...
def make_final_scoring_heatmap(dat_x, num_players):
    ...
    for f in factions:
        ...
        if not 'final_scoring' in dat_json[str(num_players)][f].keys():
            dat_json[str(num_players)][f]['final_scoring'] = dict()
            for fs1 in final_scorings:
                dat_json[str(num_players)][f]['final_scoring'][fs1] = dict()
                for fs2 in final_scorings:
                    if not fs1 == fs2:
                        dat_json[str(num_players)][f]['final_scoring'][fs1][fs2] = dict()
                        dat_json[str(num_players)][f]['final_scoring'][fs1][fs2]['points'] = 0
                        dat_json[str(num_players)][f]['final_scoring'][fs1][fs2]['vict_prob'] = 0
                        dat_json[str(num_players)][f]['final_scoring'][fs1][fs2]['delta_vict_prob'] = 0
    ...
    for faction in factions:
        ...
        for pos in range(num_players):
            ...
            for i, scoring_1 in enumerate(final_scorings):
                ...
                sc1_1_all = dat_clean[dat_clean['final_scoring_1'] == scoring_1]
                sc1_2_all = dat_clean[dat_clean['final_scoring_2'] == scoring_1]
        ...
        """
        faction_ave_pos = len(factions_pos[0]) / len(factions_in)
        ...
        for i in range(len(scores)):
            for j in range(len(scores[i])):
                if scores[i][j] is not None:
                    ...
                    # write to json
                    dat_json[str(num_players)][faction]['final_scoring'][final_scorings[i]][final_scorings[j]][
                        'points'] = scores[i][j]
                    dat_json[str(num_players)][faction]['final_scoring'][final_scorings[i]][final_scorings[j]][
                        'vict_prob'] = ave_pos[i][j]
                    dat_json[str(num_players)][faction]['final_scoring'][final_scorings[i]][final_scorings[j]][
                        'delta_vict_prob'] = ave_pos_min_faction[i][j]
...
def make_corr_plots(dat_x, num_players):
    ...
    for pos in range(num_players):
        # sum round scorings
        dat_clean['pos_' + str(pos + 1) + '_score_round'] = 0
        for i in range(6):
            dat_clean['pos_' + str(pos + 1) + '_score_round'] += dat_clean[
                'pos_' + str(pos + 1) + '_score_round' + str(i + 1)]
        # sum final scorings
        dat_clean['pos_' + str(pos + 1) + '_score_final'] = dat_clean['pos_' + str(pos + 1) + '_score_final1'] + \
                                                            dat_clean[
...
def make_round_scoring_heatmap(dat_x, num_players):
    ...
    for f in factions:
        ...
        if 'round_scoring' not in dat_json[str(num_players)][f].keys():
            dat_json[str(num_players)][f]['round_scoring'] = dict()

        for s in score_list:
            if not s in dat_json[str(num_players)][f]['round_scoring'].keys():
                dat_json[str(num_players)][f]['round_scoring'][s] = dict()
            for r in range(6):
                rs = str(r + 1)
                if not rs in dat_json[str(num_players)][f]['round_scoring'][s].keys():
                    dat_json[str(num_players)][f]['round_scoring'][s][rs] = dict()
                    dat_json[str(num_players)][f]['round_scoring'][s][rs]['points'] = 0
                    dat_json[str(num_players)][f]['round_scoring'][s][rs]['vict_prob'] = 0
                    dat_json[str(num_players)][f]['round_scoring'][s][rs]['delta_vict_prob'] = 0
        ...
        for pos in range(num_players):
            ...
            for i, scoring in enumerate(score_list):
                for j in range(6):
                    ...
                    s_all = dat_clean[dat_clean['round_' + str(j + 1) + '_scoring'] == scoring]
                    if scores[i][j] is None:
                        mean = s['pos_' + str(pos + 1) + '_score_round' + str(j + 1)].mean()
            ...
            for j in range(6):
                if scores[i][j] is not None:
                    ...
                    if not np.isnan(scores[i][j]):
                        dat_json[str(num_players)][faction]['round_scoring'][score_list[i]][str(j + 1)][
                            'points'] = scores[i][j]
                    else:
                        dat_json[str(num_players)][faction]['round_scoring'][score_list[i]][str(j + 1)][
                            'points'] = 0
